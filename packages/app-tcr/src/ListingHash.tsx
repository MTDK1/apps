...
class ListingHash extends React.PureComponent<Props, State> {
  ...
  public static getCachedComponent(listingIdx: number): CacheInstance {
    ...
    if (!cache[listingIdx]) {
      ...
      const Component = renderHelper(
        ...
        (value: any): React.ReactNode => {
          if (value) {
            console.log("ListingHash >", u8aToHex(value.toU8a(true), -1));
            return (
              <Listings value={{ id: listingIdx, key: api.query.tcr.listings, params: [u8aToHex(value.toU8a(true), -1)] }}/>
            )
          } else {
  ...
  public static getDerivedStateFromProps({ listingIdx }: Props): Pick<State, never> {

    console.log("listingIdx", listingIdx);
    const Component = ListingHash.getCachedComponent(Number(listingIdx)).Component;
